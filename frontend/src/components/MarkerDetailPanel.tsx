...
export function MarkerDetailPanel({ pin, creatorName, onClose }: Props) {
  ...
  return (
    <div className="absolute left-3 right-3 top-20 z-20 w-auto overflow-hidden rounded border border-[#2D4A2D]/15 bg-[#F5F0E8]/97 shadow-xl backdrop-blur sm:left-auto sm:right-4 sm:w-[min(calc(100%-2rem),320px)]">
      <div className="flex items-center justify-between border-b border-[#2D4A2D]/10 px-4 py-3">
        <h3 className="m-0 font-[var(--font-display)] text-lg text-[#2D4A2D]">{pin.title}</h3>
        <button type="button" onClick={onClose} className="rounded bg-[#EDEAE0] p-1.5 text-[#2D4A2D]">
          <X size={16} />
        </button>
      </div>
